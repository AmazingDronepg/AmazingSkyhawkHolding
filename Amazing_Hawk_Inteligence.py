...
def calcular_cenarios_fiscais(faturamento_mensal, empresa_nome):
    ...
    aliq_simples = 0.06; faixa = "1 (Até 180k)"
...
def calcular_totais(carrinho):
    ...
    for item in carrinho:
        ...
        if "Monitoramento" in item['nome']: fat_sky += item['valor_total']
        else: fat_amz += item['valor_total']
    
    if fat_sky > 0 and fat_amz > 0: empresa = "CONTRATO HÍBRIDO"
    elif fat_amz > 0: empresa = "AmazingDrone Solutions"
    else: empresa = "SkyHawk Security"
...
def gerar_proposta_pdf(cliente, contrato, duracao, carrinho, total, roi_data):
    ...
    pdf.ln(15)
    pdf.set_font("Arial", 'B', 10)
    pdf.set_fill_color(0, 77, 64); pdf.set_text_color(255, 255, 255)
    ...
    # Assinaturas
    pdf.set_y(-45); y_sig = pdf.get_y(); pdf.set_font("Arial", '', 8)
    pdf.line(10, y_sig, 65, y_sig); pdf.text(15, y_sig+5, "Diretoria SkyHawk Security")
    pdf.line(75, y_sig, 130, y_sig); pdf.text(80, y_sig+5, "Engenharia AmazingDrone")
    pdf.line(140, y_sig, 195, y_sig); pdf.text(145, y_sig+5, "De Acordo (Cliente)")
...
def gerar_relatorio_geral_completo_pdf(df):
    ...
    pdf.set_font("Arial", 'B', 14); pdf.set_text_color(0, 77, 64)
...
def main():
    ...
    if menu == "Nova Proposta":
            ...
            with st.container(border=True):
                ...
                if servico == "Monitoramento":
                    if "Comodato" in contrato:
                        tipo_preco = "Comodato"
                        if duracao == 12:
                            preco_base = 46000.00; label_duracao = "1 Ano (Alto Risco + 30% Margem)"
                        elif duracao == 24:
                            preco_base = 34000.00; label_duracao = "2 Anos"
                        elif duracao == 36:
                            preco_base = 30000.00; label_duracao = "3 Anos (Padrão)"
                        elif duracao == 48:
                            preco_base = 28000.00; label_duracao = "4 Anos"
                        else:
                            preco_base = 26000.00; label_duracao = "5 Anos"
                    else:
                        tipo_preco = "SaaS (Venda)"
                        if duracao == 12:
                            preco_base = 26000.00; label_duracao = "1 Ano"
                        elif duracao == 24:
                            preco_base = 24000.00; label_duracao = "2 Anos"
                        elif duracao == 36:
                            preco_base = 22000.00; label_duracao = "3 Anos (Padrão)"
                        elif duracao == 48:
                            preco_base = 20500.00; label_duracao = "4 Anos"
                        else:
                            preco_base = 19000.00; label_duracao = "5 Anos"
                    ...
                    if st.button("Adicionar Monitoramento"):
                        st.session_state['carrinho'].append({"nome": f"Monitoramento {tipo_preco} ({label_duracao})", "qtd": qtd_tot, "unidade": "rondas", "valor_unit": valor_calc/qtd_tot, "valor_total": valor_calc})
                        st.rerun()
                    ...
                    if st.button("Adicionar Volumetria"): st.session_state['carrinho'].append({"nome": f"Volumetria ({qb} Bat)", "qtd": qv, "unidade": "vols", "valor_unit": val/qv, "valor_total": val}); st.rerun()
                else:
                    c1, c2 = st.columns(2)
                    q = c1.number_input("Qtd", 1, 100, 1)
                    v = c2.number_input("Valor", 0.0, step=100.0)
                    if st.button(f"Adicionar {servico}"): st.session_state['carrinho'].append({"nome": servico, "qtd": q, "unidade": "unid", "valor_unit": v, "valor_total": q*v}); st.rerun()
        ...
        with col2:
            ...
            if st.session_state['carrinho']:
                for i, item in enumerate(st.session_state['carrinho']):
                    ...
                    c1.write(f"**{item['nome']}**"); c2.write(f"R$ {item['valor_total']:,.2f}")
                ...
                if st.button("💾 Fechar Contrato", type="primary"):
                    ...
                    st.success("Salvo!"); st.session_state['carrinho'] = []; st.rerun()
        ...
        if not df.empty:
            ...
            st.download_button("📥 Baixar Relatório Geral Completo (PDF)", pdf_completo, "Relatorio_Geral_Completo.pdf", "application/pdf", type="primary")
        else:
